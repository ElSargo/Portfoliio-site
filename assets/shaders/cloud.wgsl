...
fn fragment(
    ...
    {// Water
        ...
        p *= 4.;
        let pd = (sun_dir.xz* vec2(1.,1.)*0.00001 + p );
        let rd = normalize(world_position.xyz - material.camera_position);
        let sun = sun_dir * vec3(-1., 1., 1.);
        let noi = 2.0 - 1.5 * abs(textureSample(v_tex, v_sampler, (p +  material.time * 0.01))*textureSample(v_tex, v_sampler, (p -  material.time * 0.01 + vec2(-2.123,1.33123))) - 0.1 ) ;
        let noid = 2.0 - 1.5 * abs(textureSample(v_tex, v_sampler, (pd +  material.time * 0.01))*textureSample(v_tex, v_sampler, (pd -  material.time * 0.01 + vec2(-2.123,1.33123))) - 0.1 ) ;
        let s = (noi - noid) * 1000.;
